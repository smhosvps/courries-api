...
// Shared internal function to avoid code duplication (optional but cleaner)
// If you truly need three separate copies, you can inline this logic into each function.
async function fetchDashboardData() {
  ...
  const createdOrders = totalOrders;
  ...
  const formattedWithdrawals = recentWithdrawals.map((w: any, idx: number) => ({
    no: idx + 1,
  ...
  // 7. Additional table data (by city)
  const cityStats = await Delivery.aggregate([
    {
      $group: {
        _id: { $substrCP: ['$pickup.address', 0, 20] },
        total: { $sum: 1 },
        inProgress: {
...
// ---------- Three separate controller functions (same logic) ----------
export const getDashboardDataV1 = async (req: Request, res: Response) => {
  try {
...
export const getDashboardDataV2 = async (req: Request, res: Response) => {
  try {
    const data = await fetchDashboardData();
    res.status(200).json({ success: true, data });
  } catch (error) {
...
export const getDashboardDataV3 = async (req: Request, res: Response) => {
  try {
    const data = await fetchDashboardData();
    res.status(200).json({ success: true, data });
  } catch (error) {
